 ...
 *** =-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-="""
 ...
 class Solution:
    def fullJustify(self, words: List[str], maxWidth: int) -> List[str]:
        ...
        line_space = 1
        line_str = ""
        for curr_sub in range(1, len(words)):
            if line_width + 1 + len(words[curr_sub]) <= maxWidth:
                line_width += 1 + len(words[curr_sub])
            else:
                space_left = maxWidth - line_width
                if (curr_sub - left_sub - 1) != 0:
                    line_space += space_left // (curr_sub - left_sub - 1)
                    mod_line_space = space_left % (curr_sub - left_sub - 1)
                for ws in range(left_sub, curr_sub):
                    line_str += words[ws]
                    if ws < curr_sub - 1:
                        line_str += " " * line_space
                        if mod_line_space > 0:
                ...
                pretty_lines.append(line_str)
                left_sub = curr_sub
                line_width = len(words[left_sub])
                line_space = 1
                line_str = ""
        line_str = ""
        for ws in range(left_sub, len(words)):
            line_str += words[ws] + " "
        if maxWidth - line_width > 0:
            line_str += " " * (maxWidth - line_width - 1)
        else:
            line_str = line_str[:-1]
        pretty_lines.append(line_str)
        return pretty_lines
